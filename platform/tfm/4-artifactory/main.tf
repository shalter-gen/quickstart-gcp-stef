resource "google_artifact_registry_repository" "my-repo" {
  ...
  description   = "Genesys MultiCloud Private Edition Container and Helm Chart Repository"
  format        = "DOCKER"
}
...
resource "null_resource" "chart-push" {
  ...
  for_each = var.charts
  provisioner "local-exec" {
    command = "helm push ${each.key}-${each.value}.tgz oci://${var.region}-docker.pkg.dev/${var.project}/${var.repoid}/charts"
  }

...
for i in {1..12}; do
  ...
  CSTAT=$(kubectl get pods -l service=ucsx,tenant=shared  --no-headers | awk '{print $3}' | sort | uniq)
  [[ $CSTAT == "Running" ]] && break
...
for pod in $(kubectl get pods -l service=ucsx,tenant=shared  --no-headers | grep Running | awk '{print $1}'); do
